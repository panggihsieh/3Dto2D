...
- 接榫齒不可從每條邊的端點直接開始排列。
- 在矩形轉角、斜邊交會處、屋脊頂點等位置，需保留角落安全距離，避免產生多餘的小凸切、小菱形切割或短線段。
- 安全距離建議至少為一個齒寬，且不可小於材料厚度與接榫深度。
- 產生接榫時，應先扣除邊線兩端的安全距離，再在剩餘可用長度內平均分配齒與間距。
- 對雙斜屋頂小屋的 `front_gable` / `back_gable` 特別重要，屋脊與斜邊轉角若未避開，會在雷切輸出中出現不必要的切割線。
- 預覽可顯示內尺寸黑線與標籤協助檢查，但 SVG / DXF 輸出只保留紅色切割線。

# 規劃注意事項：內尺寸 offset + 木板厚度

- 內尺寸模式下，黑色框表示真正的內部可用尺寸。
- 紅色切割線表示實際切割外形，應依板材厚度自動向外 offset。
- 對長方體/正方體而言，`Top` / `Bottom` 仍以內部長寬作為主要基準，接榫往外延伸。
- 圍繞 `Top` / `Bottom` 的側板高度需加入上下兩片木板厚度：`外切高度 = 內高 + 材料厚度 × 2`。
- 因此輸入內高 `60mm`、材料厚度 `3mm` 時，側板紅色切割高度應為 `60 + 3 × 2 = 66mm`，黑色內尺寸框仍為 `60mm`。
- 這樣側板不用另外複雜內縮或擴展切割面積；只要用「內尺寸黑框 + 板厚 offset + 接榫外延」即可讓學生清楚比較內尺寸與實際切割線。
...
- 自動產生瓦片、窗框、門板等裝飾細節。
- 針對特定雷射機台的專有格式輸出。
# 規劃注意事項：內尺寸黑線、offset 灰線、多邊形 offset

- 黑色線條是原先的內尺寸切片輪廓，必須保留作為尺寸對照；預覽中應加粗加黑，且確認接榫後仍保留。
- 灰色線條是板厚外擴後的 offset 參考線，未被選取的灰線不能消失。
- 只有某一條邊被點選成凸榫 `f` 或凹槽 `F`，且已由紅色接榫預覽取代時，該條對應的灰色 offset 線才可以隱藏。
- Offset 不可只支援矩形；斜頂房屋的 `front` / `back` 五邊形片，以及其他簡單封閉多邊形，也必須產生外擴灰色 offset 線。
- 匯出的 SVG / DXF 仍以實際切割線為主；黑色內尺寸線、灰色 offset 參考線、選取 overlay、標籤等只作為預覽檢查用途，除非另有明確需求，不應輸出到生產檔。
